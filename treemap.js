...
$(function(){
  ...
  var fetchCpuData = function(){
    ...
    return $.getJSON("../autoscaler/cpu2.json").then(function(data){
        cpuData = data
  ...
  $.when(fetchAllData())
   .then(function(){
      ...
      // if(showCpu) createSimpleTreemap("allcpu", allcpu)
  });
  ...
  function genChildrenRate(children){
    var myChildren =  children.slice(0)
    var colorRange = [0.1, 1.0]

    var len = myChildren.length,
        array = [],
        total = 0
    ...
    if(len > 0){
      ...
      for(var i=0; i<array.length; i++){
        var tmp = array[i]
        var colorRate = colorRange[0]
        tmp.rate = tmp.title / total

        if(tmp.value === 0){
          tmp.colorRate = colorRate
        }else{
          colorRate = tmp.used/tmp.value
